...
const Featured = () => {
  ...
  // Trigger slider to recalc width on mount, resize, or orientation change
  useEffect(() => {
    const handleResize = () => {
      sliderRef.current?.innerSlider?.onWindowResized()
    }

    handleResize() // initial fix
      ...
      <Container>
        ...
        <div className="w-full overflow-hidden">
          <Slider ref={sliderRef} {...settings}>
            {furnitureProducts.map((item) => (
              <div key={item.id} className="!w-[98%] lg:!w-[96%] relative group mx-auto lg:ml-0 ml-2.5">
                <div className="bg-[#F6F7FB]">
                  <div className="pt-8 pb-5 pl-5">
                ...
                <div className="absolute top-75 left-[50%] duration-300 ease-in-out transform -translate-x-1/2">
                  <button className='py-2 opacity-0 cursor-pointer group-hover:opacity-100 px-3 text-white text-[12px] font-main font-medium bg-[#08D15F]'>View Details</button>
                </div>
              </div>
            ))}
